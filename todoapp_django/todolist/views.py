...
class CreateTodolist(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect('/todolists/') # перенаправляем на страницу
...
class ListDetailDelete(View): #удаление списка
    def get(self, request, list_id, *args, **kwargs):
        headers = create_auth_header(request.session)
        r = requests.delete('http://127.0.0.1:8080/todolists/{}/'.format(list_id), headers=headers)
        return redirect('todolist:index')

class ListDetailUpdate(View): #изменение списка
    def get(self, request, list_id, *args, **kwargs):
...
class LoginView(View):
    def get(self, request, *args, **kwargs):
        form = LoginForm()
        return render(request, 'login.html', {'form': form}) # создаем экземпляр класса и передаем в шаблон
    def post(self, request, *args, **kwargs):                               #передаем обработчик формы входа
...
class RegisterView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid(): #создаем пользователя, устанавливаем пароль и заходим на сайт
            post_data = {'username': form.cleaned_data['username'], 'password': form.cleaned_data['password2']} #проверяем, что имя пользователя не занят и пароли совпадают
